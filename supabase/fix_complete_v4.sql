-- =============================================
-- SCRIPT V4: Com TODAS as colunas NOT NULL (name, start_date, end_date)
-- =============================================
...
-- 8. Inserir evento (name, start_date, end_date são NOT NULL)
...
BEGIN
    ...
    IF default_event_id IS NOT NULL THEN
        UPDATE public.participants SET event_id = default_event_id WHERE event_id IS NULL;
        UPDATE public.sales SET event_id = default_event_id WHERE event_id IS NULL;
        INSERT INTO public.user_events (user_id, event_id, role)
        SELECT id, default_event_id, COALESCE(role, 'closer') FROM public.users
...
UNION ALL SELECT 'Participantes migrados', COUNT(*) FROM participants WHERE event_id IS NOT NULL
UNION ALL SELECT 'Usuários com acesso', COUNT(*) FROM user_events;
